...
This is an example converter for Saki test system that only supports output in the same format as the example file. Different configurations of the test software will require an accordingly different converter.
...
| sequenceName      | SoftwareName          | If log is missing sequence name, use this one.                 |
...
* In TestXMLConverter, fill in the path to the file you want to test the converter with. There are example files in the Examples folder.
* Run SetupClient once, then you can run TestXMLConverter as many times as you want.
